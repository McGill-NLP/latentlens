"""Train a ViT-LLM connector (MLP) for LatentLens.
...
All 9 LatentLens configs freeze the LLM and vision encoder; only the
connector MLP is trained.  After training, extract the connector weights
with ``reproduce/scripts/extract_connector.py``.
...
from molmo.config import CheckpointType, TrainConfig
from molmo.data import build_train_dataloader
...
from molmo.optim import BoltOnWarmupScheduler, build_optimizer, build_scheduler, \
...
def main(cfg: TrainConfig) -> None:
    ...
    if (cfg.reset_optimizer_state or cfg.reset_trainer_state) and cfg.load_path is None:
        log.warning(
            "You want to reset the optimizer or trainer state, but we're not loading from the checkpoint. "
    ...
    # Maybe start W&B run.
    if wandb is not None and cfg.wandb is not None and (get_global_rank() == 0 or not cfg.wandb.rank_zero_only):
    ...
    if cfg.load_path is None:
        ...
        if get_local_rank() == 0:
            ...
            if cfg.initial_model_checkpoint:
                state_dict = torch.load(join(cfg.initial_model_checkpoint, "model.pt"), map_location="cpu", weights_only=False)
    ...
    # Set up device mesh for hybrid sharding
    device_mesh = None
    ...
    if cfg.fsdp.sharding_strategy in (ShardingStrategy.HYBRID_SHARD, ShardingStrategy._HYBRID_SHARD_ZERO2):
        if version.parse(torch.__version__) < version.parse("2.2.0"):
            raise OLMoConfigurationError(
                "Training does not correctly support hybrid sharding before torch 2.2.0"
            )
    ...
    with Trainer(
        ...
        if not cfg.dry_run and not cfg.no_pre_train_checkpoint and cfg.load_path is None:
            ...
            # Save a pre-train checkpoint to verify disk space, etc.
...
if __name__ == "__main__":
    ...
    try:
        from molmo.util import add_cached_path_clients
        add_cached_path_clients()
    except Exception:
        pass  # cached_path clients are optional for external users
